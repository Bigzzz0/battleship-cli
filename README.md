...
โปรเจกต์รองรับการ Build และ Deploy Frontend (Vite + React) ขึ้น GitHub Pages โดยอัตโนมัติผ่าน GitHub Actions แล้ว ขั้นตอนโดยสรุปมีดังนี้

1. **เตรียม Backend ให้พร้อมสำหรับการเข้าถึงผ่านอินเทอร์เน็ต** – URL นี้จะถูกใช้โดย Frontend ที่รันบน GitHub Pages
2. ไปที่ **Settings → Secrets and variables → Actions** ของ Repository และสร้าง Secret ใหม่ชื่อ `VITE_API_BASE_URL` ระบุ URL ของ Backend (เช่น `https://your-backend.example.com`)
3. ตรวจสอบให้แน่ใจว่า Branch `main` เป็น Branch หลัก จากนั้น Push การเปลี่ยนแปลงหรือกดรัน Workflow `Deploy Frontend to GitHub Pages` ด้วยตนเอง (ผ่านเมนู Actions → เลือก workflow → Run workflow)
4. เมื่อ Workflow สำเร็จ GitHub Pages จะถูกเปิดใช้งานอัตโนมัติ และ URL ของเว็บไซต์จะถูกแสดงในหน้า Workflow Run และใน Settings → Pages

> **หมายเหตุ:** หากต้องการทดสอบการ Build แบบ Local พร้อม base path เดียวกับ GitHub Pages ให้ตั้งค่า Environment Variable `GITHUB_ACTIONS=true` และ `GITHUB_REPOSITORY=<owner>/<repo>` ก่อนรัน `pnpm run build`
