...
* **FastAPI Backend พร้อม REST API ครบชุด:** ครอบคลุมการสร้างเกม, ดึงสถานะ (พร้อมโหมดดีบัก), ยิงกระสุน, ให้ AI ยิงตอบ, ดูประวัติ, สถิติ, ค่าสถานะของ AI รวมถึงบริการข้อมูล/ตรวจสอบการวางเรือ custom ผ่าน `/ships` และ `/ships/validate`.
* **GameService จัดการเกมแบบสองกระดาน + ประวัติ/สถิติในฝั่งเซิร์ฟเวอร์:** เซิร์ฟเวอร์ดูแลกระดานผู้เล่นและ AI แยกกัน, สลับเทิร์นอัตโนมัติ, เก็บเหตุการณ์ทุกนัดพร้อมเวลายิง และส่งคืนข้อมูลสถิติให้ UI ใช้งานได้ทันที.

* **ระบบ AI 4 ระดับพร้อมกลยุทธ์ขั้นสูงและสถิติเรียลไทม์:** ตั้งแต่ Easy ยิงสุ่มไปจนถึง Expert ที่ใช้ probability heatmap, target queue และการคาดการณ์ก้าวต่อไป พร้อมให้บริการสถิติ (hit rate, targets, predicted moves) ผ่าน API (รายละเอียดเสริมด้านล่าง).
* **React Frontend แบบ Dual-Board พร้อม Ship Placement & Feedback ครบ:** แสดงกระดานผู้เล่น/AI, ป้องกันคลิกผิดเทิร์น, มี modal วางเรือที่ตรวจสอบกับ backend, ส่วนแสดงสถิติ/ประวัติ/เสียง และแผงสถานะ AI เพื่อให้ผู้เล่นเห็นข้อมูลครบในหน้าจอเดียว.
* **หลักฐานการทดสอบอัตโนมัติ:** มีเอกสาร `documents/PYTEST_RESULTS.md` ที่รายงานผล pytest ครบชุดสำหรับ `Board` และ `GameService` เพื่อยืนยันความถูกต้องของฟีเจอร์หลักใน Sprint 4 (รายละเอียดการทดสอบอื่น ๆ ดูส่วน QA ด้านล่าง).
=======
* **ระบบ AI 4 ระดับพร้อมกลยุทธ์ขั้นสูงและสถิติเรียลไทม์:** ตั้งแต่ Easy ยิงสุ่มไปจนถึง Expert ที่ใช้ probability heatmap, target queue และการคาดการณ์ก้าวต่อไป พร้อมให้บริการสถิติ (hit rate, targets, predicted moves) ผ่าน API.
* **React Frontend แบบ Dual-Board พร้อม Ship Placement & Feedback ครบ:** แสดงกระดานผู้เล่น/AI, ป้องกันคลิกผิดเทิร์น, มี modal วางเรือที่ตรวจสอบกับ backend, ส่วนแสดงสถิติ/ประวัติ/เสียง และแผงสถานะ AI เพื่อให้ผู้เล่นเห็นข้อมูลครบในหน้าจอเดียว.
* **หลักฐานการทดสอบอัตโนมัติ:** มีเอกสาร `documents/PYTEST_RESULTS.md` ที่รายงานผล pytest ครบชุดสำหรับ `Board` และ `GameService` เพื่อยืนยันความถูกต้องของฟีเจอร์หลักใน Sprint 4.
...
* `TESTING_RESULTS.md` – รายงานสถานการณ์การทดสอบหลัก ครอบคลุมการสร้างเกม, การสลับเทิร์น, การทำงานของสถิติ และ UI/UX โดยสรุปผลว่า "ผ่าน" ทุกหัวข้อ
* `ENHANCED_TESTING_RESULTS.md` – ขยายผลทดสอบโดยเจาะลึกระบบ Expert AI และสองกระดาน
...
* `PROJECT_STRUCTURE.md` – ผังโครงสร้าง repository และคำอธิบายแต่ละโฟลเดอร์
* `Final Sprint(Week4).md` – รายงานความคืบหน้าล่าสุด เทียบกับ Sprint 3
