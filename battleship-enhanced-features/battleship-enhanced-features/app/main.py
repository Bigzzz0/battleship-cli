...
async def fire_shot(game_id: str, shot_request: ShotRequest):
    """Fire a shot at the specified position"""
    try:
        ...
        if len(position) >= 2:
            col_char = position[0]
            row_num = position[1:]
            row, col = GameUtils.parse_position(col_char, row_num)
        else:
    ...
    #To this :
        result = GameService.take_shot(game_id, position)
        if result is None:
            raise HTTPException(status_code=404, detail="Game not found")
        return result
...
async def ai_fire_shot(game_id: str):
    ...
    if shot_result is None:
        raise HTTPException(status_code=404, detail="Game not found or AI not enabled")
    return shot_result
